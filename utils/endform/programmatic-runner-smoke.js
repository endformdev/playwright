...
function publicRunner(root) {
  return requireFirst(publicRunnerCandidates(root, '.js'), 'playwright/programmatic-runner');
}

function publicRunnerCandidates(root, extension) {
  return [
    path.join(root, 'packages', 'playwright', `programmatic-runner${extension}`),
    path.join(root, `programmatic-runner${extension}`),
    path.join(root, 'node_modules', 'playwright', `programmatic-runner${extension}`),
  ];
}
...
function publicRunnerESM(root) {
  return firstExisting(publicRunnerCandidates(root, '.mjs'), 'playwright/programmatic-runner ESM entry');
}
...
`);
  ...
  try {
    ...
    assert(reporter.testTitle === 'programmatic smoke', `Expected programmatic smoke test, got ${reporter.testTitle}`);
    assert(reporter.configMetadata.fromMutatedConfig, `Expected mutated metadata, got ${JSON.stringify(reporter.configMetadata)}`);
    ...
    assert(reporter.events.includes('onStepBegin:programmatic step'), `Missing step begin. Events: ${reporter.events.join(', ')}`);
    assert(reporter.events.includes('onStepEnd:programmatic step'), `Missing step end. Events: ${reporter.events.join(', ')}`);
    assert(reporter.attachments.some(a => a.name === 'programmatic-attachment' && a.body === 'attachment-body'), `Missing attachment. Attachments: ${JSON.stringify(reporter.attachments)}`);
